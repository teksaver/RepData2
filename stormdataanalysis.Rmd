...
The last part is to normalize the significant events, as mentionned in our Exploratory Data Analysis. We choose to normalize all event types that

1. have the 5 most occurences
2. are the most economically significant
3. are the most harmfully significant
                                            ...
                                            ifelse(EVTYPE=='MARINE TSTM WIND','THUNDERSTORM WIND',
                                                   ifelse(EVTYPE=='URBAN/SML STREAM FLD','FLASH FLOOD',
                                                          ifelse(EVTYPE=='HIGH WINDS','HIGH WIND',
                                                                 as.character(EVTYPE)))))))) 
# find the 5 most economically non-normalized significant events
notNormalizedEcoCount <- storms %>%
    filter(!(tolower(EVTYPE) %in% tolower(events))) %>%
    group_by(EVTYPE) %>% 
    summarise(total=sum(ecocost)) %>%
    arrange(-total)
head(notNormalizedEcoCount,n=5)

# find the 5 most harmfully non-normalized significant events
notNormalizedHarmfulCount <- storms %>%
    filter(!(tolower(EVTYPE) %in% tolower(events))) %>%
    group_by(EVTYPE) %>% 
    summarise(total=sum(harmfulness)) %>%
    arrange(-total)
head(notNormalizedHarmfulCount,n=5)

```
We see that the only non-normalized economically significant event is unknown and that no event with impact on health remains, so nothing to do.
...
We first report the 10 most economically significant events
